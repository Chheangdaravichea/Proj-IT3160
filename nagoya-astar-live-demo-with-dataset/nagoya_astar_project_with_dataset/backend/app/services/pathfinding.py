...
NODES_CSV = DATA_DIR / "nagoya_nodes.csv"
EDGES_CSV = DATA_DIR / "nagoya_edges.csv"
...
class PathfindingService:
    def __init__(self) -> None:
        # Load the Nagoya map dataset from CSV files.
        # nodes.csv stores coordinates; edges.csv stores road connections.
        self.nodes = self.load_nodes(NODES_CSV)
        undirected_edges = self.load_edges(EDGES_CSV)
        ...
        for u, v, csv_distance_m in undirected_edges:
            if u not in self.nodes or v not in self.nodes:
                continue
            w = csv_distance_m or self.haversine(self.node_latlng(u), self.node_latlng(v))
            self.adj[u].append(v)
            self.adj[v].append(u)
    ...
    @staticmethod
    def load_nodes(path: Path) -> Dict[int, Dict[str, float | str]]:
        nodes: Dict[int, Dict[str, float | str]] = {}
    ...
    @staticmethod
    def load_edges(path: Path) -> List[Tuple[int, int, float]]:
