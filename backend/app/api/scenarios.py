...
from ..services.auth import CurrentUser, require_admin, require_user
...
def list_scenarios(current_user: CurrentUser = Depends(require_user)):
    return {"items": scenario_service.active_scenarios}


@router.post("/scenarios")
def create_scenario(payload: ScenarioRequest, current_user: CurrentUser = Depends(require_admin)):
...
def clear_scenarios(current_user: CurrentUser = Depends(require_admin)):
    scenario_service.clear_all()
    return {"message": "All scenarios cleared"}


@router.delete("/scenarios/{scenario_id}")
def delete_scenario(scenario_id: int, current_user: CurrentUser = Depends(require_admin)):
